...
def get_redshift_from_csv(galaxy_path):
    redshift = []
    flag = 0
    with open(galaxy_path) as f:
        header = csv.DictReader(f)
        for row in header:
            if float(row['redshift']) < 0.1 and flag < 30001:
...
if __name__ == "__main__":
    ...
    galaxy_redshift = get_redshift_from_csv(
        r'D:\Code\MachineLearning\Data\2020.12.15_MergerClassifier\catalog\galaxy_redshift_radec.csv')
